...
import { accountAvatarApi, ipQueryApi } from '@/api'
...
const handlePromptUpload = async () => {
  ...
  try {
    ...
    const res = await accountAvatarApi.uploadPrompt(fd)
...
const fetchPromptList = async () => {
  ...
  try {
    const res = await accountAvatarApi.getPromptList({page:1,page_size:100})
...
const handleRefUpload = async () => {
  ...
  try {
    ...
    const res = await accountAvatarApi.uploadRef(fd)
...
const fetchRefList = async () => {
  ...
  try {
    const res = await accountAvatarApi.getRefList({page:1,page_size:100})
...
const handleGenerate = async () => {
  ...
  try {
    ...
    const res = await accountAvatarApi.generate(params)
...
const fetchResults = async () => {
  ...
  try {
    ...
    const res = await accountAvatarApi.getResults(params)
...
<template>
  <div class="space-y-6">
    <div>
      <h1 class="text-2xl font-bold" :style="{color:'var(--text-primary)'}">账号头像</h1>
      <p class="mt-1 text-sm" :style="{color:'var(--text-muted)'}">上传头像提示词和参考图，生成账号头像图片</p>
    </div>
    ...
    <div v-if="activeTab==='upload_prompt'" class="card-premium p-8 space-y-6">
      <div @click="promptFileRef?.click()" :class="['border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all',promptFile?'border-primary-500 bg-primary-500/5':'border-dark-600 hover:border-primary-400']">
        <input ref="promptFileRef" type="file" accept=".doc,.docx" @change="handlePromptFileSelect" class="hidden"/>
        <div v-if="!promptFile" class="space-y-2">
          ...
          <p class="text-sm" :style="{color:'var(--text-muted)'}">.doc .docx 最大 10MB</p>
        </div>
        <div v-else class="flex items-center justify-center gap-3">
          <span class="text-sm font-medium" :style="{color:'var(--text-primary)'}">{{ promptFile.name }}</span>
          <button type="button" @click.stop="promptFile=null" class="text-red-400 px-2 py-1 rounded hover:bg-red-500/10">×</button>
        </div>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div><label class="block text-sm font-medium mb-1" :style="{color:'var(--text-secondary)'}">类别</label><input v-model="promptForm.type" placeholder="如：写实风" class="input"/></div>
        <div><label class="block text-sm font-medium mb-1" :style="{color:'var(--text-secondary)'}">名称</label><input v-model="promptForm.name" placeholder="提示词名称" class="input"/></div>
        <div><label class="block text-sm font-medium mb-1" :style="{color:'var(--text-secondary)'}">简介</label><input v-model="promptForm.jianjie" placeholder="简单描述" class="input"/></div>
      </div>
    ...
    <div v-if="activeTab==='prompt_list'" class="card-premium overflow-hidden">
      ...
      <table v-else class="w-full"><thead><tr :style="{borderBottom:'1px solid var(--border-base)'}"><th class="text-left px-6 py-3 text-sm" :style="{color:'var(--text-muted)'}">ID</th><th class="text-left px-6 py-3 text-sm" :style="{color:'var(--text-muted)'}">名称</th><th class="text-left px-6 py-3 text-sm" :style="{color:'var(--text-muted)'}">类别</th><th class="text-left px-6 py-3 text-sm" :style="{color:'var(--text-muted)'}">时间</th><th class="text-left px-6 py-3 text-sm" :style="{color:'var(--text-muted)'}">操作</th></tr></thead>
        <tbody><tr v-for="r in promptList" :key="r.id" :style="{borderBottom:'1px solid var(--border-base)'}"><td class="px-6 py-3 text-sm" :style="{color:'var(--text-muted)'}">#{{ r.id }}</td><td class="px-6 py-3 text-sm font-medium" :style="{color:'var(--text-primary)'}">{{ r.name||'未命名' }}</td><td class="px-6 py-3"><span v-if="r.type" class="badge badge-primary">{{ r.type }}</span><span v-else :style="{color:'var(--text-muted)'}">-</span></td><td class="px-6 py-3 text-sm" :style="{color:'var(--text-muted)'}">{{ formatTime(r.create_time) }}</td><td class="px-6 py-3"><button @click="genForm.prompt_id=String(r.id);activeTab='generate'" class="text-sm text-primary-500 hover:text-primary-400">用此生成</button></td></tr></tbody></table>
    </div>
    <!-- 参考图上传 -->
    <div v-if="activeTab==='upload_ref'" class="card-premium p-8 space-y-6">
      <div @click="refFileRef?.click()" :class="['border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all',refFile?'border-accent-500 bg-accent-500/5':'border-dark-600 hover:border-accent-400']">
        <input ref="refFileRef" type="file" accept="image/*" @change="handleRefFileSelect" class="hidden"/>
        <div v-if="!refFile" class="space-y-2">
          <div class="w-12 h-12 mx-auto rounded-full flex items-center justify-center" :style="{background:'var(--bg-surface-2)'}"><svg class="w-6 h-6 text-accent-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg></div>
          <p class="font-medium" :style="{color:'var(--text-primary)'}">点击上传参考图片</p>
          <p class="text-sm" :style="{color:'var(--text-muted)'}">jpg/png/webp 最大 10MB</p>
        </div>
        <div v-else class="flex items-center justify-center gap-3">
          <span class="text-sm font-medium" :style="{color:'var(--text-primary)'}">{{ refFile.name }}</span>
          <button type="button" @click.stop="refFile=null" class="text-red-400 px-2 py-1 rounded hover:bg-red-500/10">×</button>
        </div>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div><label class="block text-sm font-medium mb-1" :style="{color:'var(--text-secondary)'}">类别</label><input v-model="refForm.type" class="input"/></div>
        <div><label class="block text-sm font-medium mb-1" :style="{color:'var(--text-secondary)'}">名称</label><input v-model="refForm.name" class="input"/></div>
        <div><label class="block text-sm font-medium mb-1" :style="{color:'var(--text-secondary)'}">简介</label><input v-model="refForm.jianjie" class="input"/></div>
      </div>
    ...
    <div v-if="activeTab==='ref_list'" class="card-premium p-6">
      ...
      <div v-else class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div v-for="r in refList" :key="r.id" class="card-premium p-2 space-y-2">
          <img :src="r.zhanghao_touxiang_oss_path" class="w-full aspect-square object-cover rounded-lg"/>
    ...
    <div v-if="activeTab==='generate'" class="card-premium p-8 space-y-6">
      <p class="text-sm" :style="{color:'var(--text-muted)'}">选择头像提示词，可选参考图，生成账号头像</p>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <label class="block text-sm font-medium mb-2" :style="{color:'var(--text-secondary)'}">头像提示词 <span class="text-red-500">*</span></label>
          <select v-model="genForm.prompt_id" class="input">
            <option value="">请选择提示词</option>
            <option v-for="p in promptList" :key="p.id" :value="String(p.id)">#{{ p.id }} {{ p.name||'未命名' }}</option>
          </select>
        </div>
        <div>
          <label class="block text-sm font-medium mb-2" :style="{color:'var(--text-secondary)'}">参考图（可选）</label>
          <select v-model="genForm.cankao_id" class="input">
            <option value="">不使用参考图</option>
            <option v-for="r in refList" :key="r.id" :value="String(r.id)">#{{ r.id }}</option>
          </select>
        </div>
      </div>
    ...
    <div v-if="activeTab==='results'" class="card-premium overflow-hidden">
      ...
      <div v-else class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4">
        <div v-for="r in results" :key="r.id" class="card-premium p-3 space-y-3">
          <div class="flex items-center justify-between">
            <span class="text-xs" :style="{color:'var(--text-muted)'}">#{{ r.id }}</span>
            <span :class="['badge text-xs', statusClass(r.status)]">{{ statusLabel(r.status) }}</span>
          </div>
          <template v-if="r.status===2 && r.task_record">
            <img v-if="isImage(signedUrlMap[r.id]||r.task_record)" :src="signedUrlMap[r.id]||r.task_record" class="w-full aspect-square object-cover rounded-lg" @error="getSignedUrl(r)"/>
            <div v-else class="w-full aspect-square rounded-lg flex items-center justify-center" :style="{background:'var(--bg-surface-2)'}"><span class="text-xs" :style="{color:'var(--text-muted)'}">文件结果</span></div>
          ...
          <div v-else class="w-full aspect-square rounded-lg flex items-center justify-center" :style="{background:'var(--bg-surface-2)'}"><span class="text-xs" :style="{color:'var(--text-muted)'}">{{ statusLabel(r.status) }}</span></div>
