...
/**
 * IP调查问卷查询页面
 * 对接后端 /gongneng/gongneng_3 接口
 */

import { ref, onMounted } from 'vue'
import { useAppStore } from '@/stores/app'
import { ipQueryApi } from '@/api'
import type { IPQueryRecord } from '@/types/api'
import OnlineFillDialog from './OnlineFillDialog.vue'

const appStore = useAppStore()

// 列表数据
const records = ref<IPQueryRecord[]>([])
const total = ref(0)
...
const onlineFillTemplateName = ref<string | undefined>(undefined)
...
const fetchList = async () => {
  loading.value = true
  try {
    const res = await ipQueryApi.getList({ page: page.value, page_size: pageSize.value })
 ...
 * 预览/下载 - 获取签名URL
 */
const handlePreview = async (record: IPQueryRecord) => {
 ...
 * 打开在线填写弹窗
 */
const openOnlineFill = (record: IPQueryRecord) => {
  onlineFillTemplateName.value = record.name
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-white">问卷列表</h1>
    ...
    <div class="card-premium overflow-hidden">
      ...
      <div v-else-if="records.length === 0" class="flex flex-col items-center justify-center py-24 text-dark-400">
        ...
        <p class="text-sm mt-1">还没有上传任何IP调查问卷</p>
      </div>
      ...
      <table v-else class="w-full">
        <thead>
          <tr class="border-b border-dark-700">
            ...
            <th class="text-left px-6 py-4 text-sm font-medium text-dark-400">上传时间</th>
    ...
    <OnlineFillDialog
      ...
      :template-name="onlineFillTemplateName"
      @submitted="fetchList"
    />
  </div>
</template>
